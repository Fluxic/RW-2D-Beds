...
namespace TwoDBedPatch
{
	...
	{
		public static bool Is2DBed(string defName)
		{
			switch (defName)
			{
				case "AvalipillowsOne":
				case "AvalipillowsTwo":
				case "AvalipillowsThree":
					return true;
			}
			return false;
		}
		...
		public static IntVec3 Bed2D_GetSleepingSlotPos(int index, IntVec3 bedCenter, Rot4 bedRot, IntVec2 bedSize)
		{
			...
			{
				return new IntVec3(cellRect.minX + index % bedSize.x, bedCenter.y, cellRect.minZ + index / bedSize.x);
			}
			else if (bedRot == Rot4.East)
			{
				return new IntVec3(cellRect.minX + index / bedSize.x, bedCenter.y, cellRect.maxZ - index % bedSize.x);
			}
		...
		{
			if (Is2DBed(__instance.def.defName))
			{
